...
function main() {
  ...
  assertMatches(indexHtml, /\.mira-intro \.pet\s*\{[\s\S]*--mira-intro-avatar-scale:\s*1\.28;[\s\S]*--mira-intro-avatar-offset:\s*var\(--ef-space-1\);[\s\S]*width:\s*calc\(var\(--ef-mira-avatar-size\) \+ var\(--ef-space-3\)\);[\s\S]*height:\s*calc\(var\(--ef-mira-avatar-size\) \+ var\(--ef-space-3\)\);[\s\S]*border-radius:\s*var\(--ef-radius-pill\);[\s\S]*radial-gradient\(circle at 50% 54%/, "installed onboarding Mira floats on a soft glow while keeping canonical face geometry");
  ...
  assertMatches(indexHtml, /\.break-overlay\.feedback-mode \.break-mira \.pet\s*\{[\s\S]*--feedback-mira-avatar-offset:\s*var\(--ef-space-1\);[\s\S]*width:\s*calc\(var\(--ef-mira-avatar-size\) \+ var\(--ef-space-3\)\);[\s\S]*border-radius:\s*var\(--ef-radius-pill\);[\s\S]*radial-gradient\(circle at 50% 54%, rgba\(225,\s*255,\s*244,\s*0\.4\)/, "installed feedback Mira day glow stays soft (dialed down for the small corner avatar)");
  assertMatches(indexHtml, /:root\[data-theme="dark"\] \.break-overlay\.feedback-mode \.break-mira \.pet\s*\{[\s\S]*radial-gradient\(circle at 50% 52%, rgba\(79,\s*201,\s*156,\s*0\.12\)/, "installed feedback Mira night glow stays soft (dialed down for the small corner avatar)");
  ...
  assertMatches(breakLockHtml, /(?:^|\n)\s*\.pet\s*\{[\s\S]*width:\s*var\(--ef-mira-avatar-size\);[\s\S]*border-radius:\s*50%;[\s\S]*radial-gradient\(circle at 50% 50%[\s\S]*transform:\s*scale\(2\.12\);/, "installed break lock Mira uses a circular avatar shell");
  ...
  assertMatches(indexHtml, /#profileView \.share-art-mark\s*\{[\s\S]*width:\s*calc\(var\(--ef-control-lg\) \+ var\(--ef-space-1\)\);[\s\S]*background:\s*url\("\.\/assets\/icon\.svg"\) center \/ contain no-repeat;/, "installed profile share card uses the source app icon asset");

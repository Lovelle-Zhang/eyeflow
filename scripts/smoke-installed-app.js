...
function main() {
  ...
  assertMatches(indexHtml, /\.break-overlay\.feedback-mode \.break-mira \.pet\s*\{[\s\S]*--feedback-mira-avatar-offset:\s*var\(--ef-space-1\);[\s\S]*width:\s*calc\(var\(--ef-mira-avatar-size\) \+ var\(--ef-space-3\)\);[\s\S]*border-radius:\s*var\(--ef-radius-pill\);[\s\S]*background:\s*#edf3ef;/, "installed feedback Mira is a solid round avatar (no glow)");

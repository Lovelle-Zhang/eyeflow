...
function main() {
  ...
  assertMatches(mainJs, /function applyInterventionBehavior\(state\)[\s\S]*const showBubble = companionVisible && level >= 2;[\s\S]*const showIsland = islandEnabled && \(companionExited \|\| level >= 3\);[\s\S]*const showNotify = companionExited && level >= 2[\s\S]*&& \(level >= 3 \|\| !islandEnabled\);[\s\S]*if \(showBubble\) \{[\s\S]*showCompanionPanel\(\);[\s\S]*if \(showIsland\) showNotchIsland\(level >= 2 \? reminderMessage : presenceMessage\);[\s\S]*if \(showNotify\) notify\(reminderMessage\);/, "installed reminder channels escalate by level on one shared cooldown");
  assertMatches(mainJs, /showNotchIsland\(level >= 2 \? reminderMessage : presenceMessage\)/, "installed L1 island shows a gentle presence line");

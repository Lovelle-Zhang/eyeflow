...
function main() {
  ...
  assertMatches(indexHtml, /\.break-overlay\.feedback-mode \.break-mira \.pet\s*\{[\s\S]*--feedback-mira-avatar-offset:\s*var\(--ef-space-1\);[\s\S]*width:\s*calc\(var\(--ef-mira-avatar-size\) \+ var\(--ef-space-3\)\);[\s\S]*border-radius:\s*var\(--ef-radius-pill\);[\s\S]*radial-gradient\(circle at 50% 54%, rgba\(225,\s*255,\s*244,\s*0\.58\)/, "installed feedback Mira day glow is synced to onboarding");
  assertMatches(indexHtml, /:root\[data-theme="dark"\] \.break-overlay\.feedback-mode \.break-mira \.pet\s*\{[\s\S]*radial-gradient\(circle at 50% 52%, rgba\(79,\s*201,\s*156,\s*0\.18\)/, "installed feedback Mira night glow is synced to onboarding");

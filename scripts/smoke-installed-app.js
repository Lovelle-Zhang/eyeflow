...
function main() {
  ...
  assertMatches(mainJs, /function applyInterventionBehavior\(state\)[\s\S]*const showBubble = companionVisible && level >= 2;[\s\S]*const showRest = islandEnabled && companionExited && level >= 2;[\s\S]*const breakBypass = breakDue && !breakRestSurfaced;[\s\S]*if \(escalated \|\| breakBypass \|\| now - lastReminderAt > reminderCooldown\) \{[\s\S]*if \(breakDue\) breakRestSurfaced = true;[\s\S]*if \(showRest\) \{[\s\S]*if \(breakDue\) \{[\s\S]*startIslandMicroRest\(islandRestMessage\(level\), state\.reminderId \|\| null\);[\s\S]*showNotchIsland\(\{ mode: "text", message: islandNoticeMessage\(state, level\) \}\);/, "installed: real break point gets countdown + cooldown bypass; pre-target is a no-countdown heads-up");

...
function main() {
  ...
  assertIncludes(indexHtml, "border-top: 1px solid rgba(24, 32, 31, 0.045);", "installed settings preference rows avoid card borders");
  ...
  assertMatches(indexHtml, /#rhythmView \.readiness-item \.settings-action-button\s*\{[\s\S]*min-height:\s*var\(--ef-control-md\);[\s\S]*padding-inline:\s*var\(--ef-space-3\);/, "installed settings desktop readiness buttons use the smaller default control size");
  ...
  assertMatches(indexHtml, /\.session-state-pill\s*\{[\s\S]*width:\s*fit-content;[\s\S]*border-radius:\s*var\(--ef-radius-pill\);[\s\S]*min-height:\s*var\(--ef-control-sm\);[\s\S]*font-size:\s*var\(--ef-text-meta\);/, "installed session status pill uses tokenized shape and type");
  assertMatches(indexHtml, /\.timer-card > \.timer-controls \.primary,[\s\S]*\.timer-card > \.timer-controls \.ghost\s*\{[\s\S]*min-height:\s*var\(--ef-control-lg\);/, "installed session primary controls use design-system height");
  ...
  assertMatches(indexHtml, /\.state-action-row \.primary\s*\{[\s\S]*min-height:\s*var\(--ef-control-lg\);[\s\S]*color:\s*#ffffff;[\s\S]*background:\s*var\(--mira\);/, "installed today primary action is a filled Mira-green button");
  ...
  assertMatches(indexHtml, /#profileView \.profile-share-bridge::before,[\s\S]*#profileView \.profile-share-bridge::after\s*\{[\s\S]*height:\s*1px;[\s\S]*background:\s*rgba\(24,\s*32,\s*31,\s*0\.055\);/, "installed profile share transition uses quiet divider lines");

...
function main() {
  ...
  assertIncludes(indexHtml, "border-top: 1px solid rgba(24, 32, 31, 0.045);", "settings preference rows avoid card borders");
  ...
  assertMatches(indexHtml, /#rhythmView \.readiness-item \.settings-action-button\s*\{[\s\S]*min-height:\s*var\(--ef-control-md\);[\s\S]*padding-inline:\s*var\(--ef-space-3\);/, "settings desktop readiness buttons use the smaller default control size");

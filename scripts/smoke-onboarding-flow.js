...
function main() {
  ...
  // Reminder channels are coordinated on ONE shared cooldown: L2 stays in Mira when
  // visible, while L3 at the real break point becomes explicit with the top capsule
  // plus the system notification.
  // One line, not a matrix: how strong (level) and where Mira can deliver (visibility) are
  // separate. Mira on-screen → her bubble carries L2+. At L3's real break point, the
  // green capsule + system notification become the clear prompt even if Mira is visible.
  // Mira exited → the island IS the reminder: a self-closing look-away micro-rest.
  // 2026-07-10 stopgap: delivery is transactional (deliver first, consume latch on
  // confirmation) and banners go through the throttled notifyReminder — this pattern
  // guards the NEW shape; the old one asserted latch-before-delivery + raw notify.
  assertMatches(mainJs, /function surfaceReminderChannels\(decision\)[\s\S]*?if \(showRest\) \{[\s\S]*?if \(breakDue\) \{[\s\S]*?restDelivered = startIslandMicroRest\(islandRestMessage\(level\), reminderId \|\| null\);[\s\S]*?showNotchIsland\(\{ mode: "text", message: islandNoticeMessage\(\{ forceMode \}, level\) \}\);[\s\S]*?if \(showNotify\) \{[\s\S]*?const sent = notifyReminder\(reminderMessage, \{ urgent: breakDue && !showRest \}\);[\s\S]*?function applyInterventionBehavior\(state\)[\s\S]*?const breakDue = Boolean\(state\.breakDue\);[\s\S]*?const l3BreakPoint = level >= 3 && breakDue;[\s\S]*?const showBubble = companionVisible && level >= 2 && !l3BreakPoint;[\s\S]*?const showRest = islandEnabled && level >= 2 && \(companionExited \|\| l3BreakPoint\);[\s\S]*?const showNotify = level >= 2 && \(companionExited \|\| l3BreakPoint\) && \(level >= 3 \|\| !islandEnabled\);[\s\S]*?const breakBypass = breakDue && !breakRestSurfaced;[\s\S]*?if \(escalated \|\| breakBypass \|\| now - lastReminderAt > reminderCooldown\) \{[\s\S]*?const surfaced = surfaceReminderChannels\(\{[\s\S]*?if \(delivered\) \{\s*lastReminderAt = now;[\s\S]*?lastStableInterventionLevel = level;\s*pendingEscalationLevel = 0;\s*\}[\s\S]*?if \(breakDue\) breakRestSurfaced = true;/, "L3 real break point gets green capsule + system notification even when Mira is visible");
  assertIncludes(indexHtml, "const breakDue = Boolean(intervention.breakDue);", "renderer publishes breakDue straight from the pressure-engine intent (P3) — elapsed/naturalBreak heuristics retired");
  // P3: the pressure engine counts continuous eye time regardless of manual/auto
  // session mode, so force reaches level 4 through the same hard-full intent for
  // both — the translation branch must not consult isRunning/isAutoTracking.
  assertMatches(indexHtml, /if \(intent\.surface === "hard-full"\) \{\s*return \{\s*level: 4,[\s\S]*?title: "强制爱：全屏休息"/, "L4 automatic tracking reaches the same full-screen recovery branch as manual timing (hard-full intent)");
  assertMatches(indexHtml, /if \(!intervention\.breakDue\) return;/, "reminder recording keys off the engine intent alone — auto tracking surfaces reminders identically (P3)");
  assertMatches(mainJs, /function islandNoticeMessage\(state, level\)[\s\S]*state\.forceMode[\s\S]*进入全屏恢复[\s\S]*快到断点了/, "the pre-target heads-up has its own no-countdown copy; force mode warns of the full-screen takeover");
  // P3: the takeover pre-warning now rides the level-2 micro intent — force copy
  // explains what happens if the rest keeps being skipped, so the hard-full
  // takeover is never abrupt.
  assertMatches(indexHtml, /copy: intensity === "force"\s*\?\s*`已经连续用眼 \$\{minutes\} 分钟。先跟着歇 20 秒；再攒下去、两次没歇，会进入全屏恢复。`/, "L4 强制爱 gets a pre-takeover heads-up on the level-2 intent so the full-screen takeover is never abrupt");

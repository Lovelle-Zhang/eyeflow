...
function main() {
  ...
  assertMatches(mainJs, /function surfaceReminderChannels\(decision\)[\s\S]*?if \(showRest\) \{[\s\S]*?if \(breakDue\) \{[\s\S]*?restDelivered = startIslandMicroRest\(islandRestMessage\(level\), reminderId \|\| null\);[\s\S]*?if \(showNotify\) \{[\s\S]*?const sent = notifyReminder\(reminderMessage, \{ urgent: breakDue && !showRest \}\);[\s\S]*?function applyInterventionBehavior\(state\)[\s\S]*?const breakDue = Boolean\(state\.breakDue\);[\s\S]*?const showBubble = companionVisible && level < 3;[\s\S]*?const showRest = islandEnabled && !deepWorkQuiet && \(companionExited \|\| level >= 3\);[\s\S]*?const showNotify = !deepWorkQuiet && \(companionExited \|\| level >= 3\) && \(level >= 3 \|\| !islandEnabled\);[\s\S]*?const intentKey = `\$\{level\}\|\$\{surface\}\|\$\{breakDue\}`;[\s\S]*?const surfaced = surfaceReminderChannels\(\{[\s\S]*?if \(delivered\) lastReminderAt = now;[\s\S]*?lastDeliveredIntentKey = intentKey;/, "level-3 intent gets green capsule + system notification even when Mira is visible (P3 intent-key delivery)");

...
function main() {
  ...
  assertMatches(indexHtml, /\.onboarding-dialog\s*\{[\s\S]*width:\s*min\(calc\(var\(--ef-space-14\) \* 9\.4\),\s*100%\);[\s\S]*padding:\s*var\(--ef-space-8\) var\(--ef-space-8\) var\(--ef-space-6\);[\s\S]*box-shadow:\s*0 0 0 0\.5px rgba\(0,\s*0,\s*0,\s*0\.06\),\s*0 12px 32px rgba\(0,\s*0,\s*0,\s*0\.12\);/, "onboarding dialog keeps bottom whitespace restrained with a quieter ADA shadow");
  ...
  assertMatches(indexHtml, /\.mira-intro \.pet\s*\{[\s\S]*--mira-intro-avatar-scale:\s*1\.24;[\s\S]*width:\s*calc\(var\(--ef-space-14\) \+ var\(--ef-space-7\)\);[\s\S]*height:\s*calc\(var\(--ef-space-14\) \+ var\(--ef-space-7\)\);[\s\S]*border-radius:\s*var\(--ef-radius-pill\);[\s\S]*radial-gradient\(circle/, "onboarding Mira reads as a larger round avatar instead of an app icon tile");
  ...
  assertMatches(indexHtml, /\.onboarding-actions \.primary\s*\{[\s\S]*min-height:\s*var\(--ef-control-lg\);[\s\S]*box-shadow:\s*none;/, "onboarding keeps only one calm primary action");

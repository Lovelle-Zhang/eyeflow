...
function main() {
  ...
  assertMatches(indexHtml, /\.mira-intro \.pet\s*\{[\s\S]*--mira-intro-avatar-scale:\s*1\.28;[\s\S]*--mira-intro-avatar-offset:\s*var\(--ef-space-1\);[\s\S]*width:\s*calc\(var\(--ef-mira-avatar-size\) \+ var\(--ef-space-3\)\);[\s\S]*height:\s*calc\(var\(--ef-mira-avatar-size\) \+ var\(--ef-space-3\)\);[\s\S]*border-radius:\s*var\(--ef-radius-pill\);[\s\S]*radial-gradient\(circle at 50% 54%/, "onboarding Mira floats on a soft glow while keeping canonical face geometry");

...
function main() {
  ...
  assertMatches(mainJs, /function applyInterventionBehavior\(state\)[\s\S]*const breakDue = Boolean\(state\.breakDue\);[\s\S]*const l3BreakPoint = level >= 3 && breakDue;[\s\S]*const showBubble = companionVisible && level >= 2 && !l3BreakPoint;[\s\S]*const showRest = islandEnabled && level >= 2 && \(companionExited \|\| l3BreakPoint\);[\s\S]*const showNotify = level >= 2 && \(companionExited \|\| l3BreakPoint\) && \(level >= 3 \|\| !islandEnabled\);[\s\S]*const breakBypass = breakDue && !breakRestSurfaced;[\s\S]*if \(escalated \|\| breakBypass \|\| now - lastReminderAt > reminderCooldown\) \{[\s\S]*if \(showRest\) \{[\s\S]*if \(breakDue\) \{[\s\S]*restDelivered = startIslandMicroRest\(islandRestMessage\(level\), state\.reminderId \|\| null\);[\s\S]*showNotchIsland\(\{ mode: "text", message: islandNoticeMessage\(state, level\) \}\);[\s\S]*if \(showNotify\) \{[\s\S]*const sent = notifyReminder\(reminderMessage, \{ urgent: breakDue && !showRest \}\);[\s\S]*if \(delivered\) lastReminderAt = now;[\s\S]*if \(breakDue\) breakRestSurfaced = true;/, "L3 real break point gets green capsule + system notification even when Mira is visible");

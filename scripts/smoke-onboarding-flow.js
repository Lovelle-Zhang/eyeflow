...
function main() {
  ...
  assertIncludes(indexHtml, "const breakDue = Boolean(((isRunning || isAutoTracking()) && Number(elapsedSeconds) >= Number(els.focusTarget.value) * 60) || naturalBreak);", "renderer publishes breakDue = manual or automatic timer hit target (or natural break) so main can distinguish the real break point");
  assertMatches(indexHtml, /function\s+currentIntervention\(load\)[\s\S]*if \(!\(isRunning \|\| isAutoTracking\(\)\)\) \{[\s\S]*title: "强制爱待命"[\s\S]*if \(elapsedMinutes >= focusTargetMinutes\) \{[\s\S]*level: 4,/, "L4 automatic tracking reaches the same full-screen recovery branch as manual timing");
  assertMatches(indexHtml, /function\s+shouldSurfaceReminder\(intervention, load\)[\s\S]*if \(!\(isRunning \|\| isAutoTracking\(\)\)\) return false;[\s\S]*return elapsedSeconds >= targetSeconds;/, "automatic tracking can surface L4 pre-target and break-point reminders");
  ...
  assertMatches(indexHtml, /elapsedMinutes >= focusTargetMinutes \* 0\.72\) \{\s*return \{\s*level: 2,\s*displayLevel: 4,\s*title: "快到恢复断点"/, "L4 强制爱 gets a pre-target heads-up (预告) so the full-screen takeover is never abrupt");
